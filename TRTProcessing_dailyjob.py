...
# Define date range
START_DATE = datetime.strptime('2019-06-21', '%Y-%m-%d')
END_DATE = datetime.strptime('2019-06-30', '%Y-%m-%d')

# Create a list of all dates in the range
daylist = [(START_DATE + timedelta(days=i)).strftime('%Y-%m-%d') 
...
def process_day(selected_date):
    ...
    valid_date3 = selected_date.strftime('%Y-%m-%d')
    date_obj = datetime.strptime(valid_date3, '%Y-%m-%d')
    year_last_two = date_obj.strftime('%y')
    day_of_year = date_obj.timetuple().tm_yday
    valid_date4 = f"{year_last_two}{day_of_year:03d}"
    ...
    try:
        ...
        current_time = datetime.combine(selected_date, datetime.strptime('0000', '%H%M').time())
        end_time = datetime.combine(selected_date, datetime.strptime('2355', '%H%M').time())
        time_delta = timedelta(minutes=5)

        while current_time <= end_time:
            valid_time3 = current_time.strftime('%H%M')

            # Process gust markers (assuming this is a custom function you have defined)
        ...
        return  # Skip processing if the ZIP file is missing
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process data for a specific day")
    parser.add_argument('-d', '--day', type=int, required=True, help="Day index (0-based)")
    args = parser.parse_args()

    # Get the selected date from the list of days
    day_index = args.day
    if day_index < 0 or day_index >= len(daylist):
        raise ValueError(f"Invalid day index. Must be between 0 and {len(daylist) - 1}.")

    selected_date_str = daylist[day_index]
    selected_date = datetime.strptime(selected_date_str, '%Y-%m-%d')

    print(f"Processing data for date: {selected_date_str}")
    process_day(selected_date)
